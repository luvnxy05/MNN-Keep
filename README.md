...
- **服务控制台**：启动即见服务状态、模型、端口、API Key、一键启停；模型自选（下载历史 + 文件扫描，手动刷新），**运行中热切换模型**（服务不中断）
- **运维面板**：请求统计（总数/成功率/Token 用量，重启保留）、运行时长、模型加载耗时、API 请求日志（API 控制台实时查看）、崩溃日志本机查看
- **部署辅助**：设备内存检测 + 模型档位建议、充电常亮（含 Android 15 权限降级引导）、厂商保活引导（自动识别 ROM 显示白名单步骤）、配置备份/恢复（JSON 一键迁移换机）
- **保活四件套**（无人值守核心）：
