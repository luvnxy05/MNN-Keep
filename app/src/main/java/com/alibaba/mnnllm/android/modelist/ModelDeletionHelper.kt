...
object ModelDeletionHelper {
    ...
    fun deleteModelWithCleanup(context: Context, modelId: String): DeletionResult {
        ...
        val modelDeleted = try {
            ...
            // If the deleted model was the persisted service model, drop the
            // reference so the console / API service doesn't point at a
            // non-existent model after deletion.
            if (CurrentModelManager.getCurrentModelId() == modelId) {
