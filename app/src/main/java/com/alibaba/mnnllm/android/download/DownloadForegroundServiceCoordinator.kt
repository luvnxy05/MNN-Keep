            ...
            override fun show(downloadCount: Int, modelName: String?): Boolean {
                return runCatching {
                    ...
                    if (service != null) {
                        ...
                        // Plain startService() + startForeground() inside the
                        // service. Using startForegroundService() here races:
                        // if the download fails instantly and stop() runs
                        // before the service calls startForeground(), Android
                        // throws ForegroundServiceDidNotStartInTimeException
                        // and kills the app (observed on Android 15).
                        // startService() has no such hard 5s contract; the
                        // service promotes itself to foreground in onCreate().
