...
class OpenAIService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (!requestedModelId.isNullOrBlank()) {
            ...
            // Auto-restart path (crash recovery / boot): restore persisted model
            val persisted = CurrentModelManager.getCurrentModelId()
            if (!persisted.isNullOrBlank()) {
                currentModelId = persisted
                Timber.tag(TAG).i("Restored modelId from persistence: $persisted")
            }
        }
        ...
        serviceScope.launch {
            ...
            if (!startSuccess && !requestedModelId.isNullOrBlank()) {
                ...
                // previousModelId == null: keep the persisted model config
                // untouched. Clearing it here (old behavior) created a
                // user-visible death loop: model deleted -> load fails ->
                // prefs wiped -> console can never start the service again.
                Timber.tag(TAG).w("Failed to switch service runtime model (previousModelId: $previousModelId, persisted config kept)")
            }
        }
        return START_STICKY
    }
    ...
    private fun cleanup() {
        ...
        // Keep the persisted model ID across service stop/restart: for a
        // headless inference box the model is a device configuration, not a
        // transient session property (official code cleared it here, which
        // broke the console's ability to restart the service unattended).
